...
import { enterpriseBundleCenters, type EnterpriseCenterKey } from "@/data/enterprise-bundle-44-48";
...
export function EnterpriseBundleCenter({ center }: Props) {
  ...
  return <main className={styles.page}>
    <section className={styles.hero}><div><span>{config.eyebrow}</span><h1>{config.title}</h1><p>{config.description}</p></div><div className={styles.heroCard}><small>{config.heroLabel}</small><strong>{config.heroValue}</strong><span>AVOS Intelligence</span></div></section>
    ...
    <section className={styles.controls}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث في البيانات والتوصيات"/><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">كل الحالات</option>{Object.entries(statusLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select><button type="button" onClick={() => { setQuery(""); setStatus("all"); }}>إعادة الضبط</button></section>
    <section className={styles.workspace}><div className={styles.listPanel}><div className={styles.heading}><div><span>ENTERPRISE INTELLIGENCE MATRIX</span><h2>المؤشرات التنفيذية</h2></div><strong>{visibleRecords.length}</strong></div><div className={styles.recordGrid}>{visibleRecords.map((record) => <button key={record.id} type="button" onClick={() => setSelectedId(record.id)} className={`${styles.recordCard} ${selected?.id === record.id ? styles.selected : ""}`}><div className={styles.cardTop}><span>{record.id}</span><span className={styles[record.status]}>{statusLabels[record.status]}</span></div><small>{record.owner}</small><h3>{record.title}</h3><p>{record.summary}</p><div className={styles.score}><div><span>الدرجة</span><strong>{record.score}%</strong></div><i><b style={{ width: `${record.score}%` }}/></i></div><footer className={styles.cardFooter}><span>{record.impact}</span><strong>{record.action}</strong></footer></button>)}</div></div>
