import { Body, Controller, Get, Param, Post } from "@nestjs/common";
import { DependencyImpactService } from "./impact.service";
import { DependencyImpactRecord } from "./impact.types";

@Controller("enterprise-dependency-graph/impact")
export class DependencyImpactController {
  constructor(private readonly service: DependencyImpactService) {}

  @Get("status") status() { return this.service.status(); }
  @Get("records") list() { return this.service.list(); }
  @Get("records/:id") get(@Param("id") id: string) { return this.service.get(id); }
  @Post("records") create(@Body() body: Pick<DependencyImpactRecord, "name" | "description"> & Partial<Pick<DependencyImpactRecord, "score" | "dependency">>) { return this.service.create(body); }
  @Post("records/:id/activate") activate(@Param("id") id: string) { return this.service.activate(id); }
  @Post("records/:id/archive") archive(@Param("id") id: string) { return this.service.archive(id); }
  @Get("records/:id/evaluate") evaluate(@Param("id") id: string) { return this.service.evaluate(id); }
}