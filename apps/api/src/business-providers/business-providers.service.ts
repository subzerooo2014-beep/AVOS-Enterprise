...
@Injectable()
export class BusinessProvidersService {
  ...
  createProvider(data: any) {
    return (this.prisma as any).businessProviderProfile.create({
      data: {
  ...
  listProviders(type?: string) {
    return (this.prisma as any).businessProviderProfile.findMany({
      where: type ? { type } : {},
  ...
  async findProvider(id: string) {
    const item = await (this.prisma as any).businessProviderProfile.findUnique({ where: { id } });
    if (!item) throw new NotFoundException("Business provider not found");
    return item;
  }

  async updateProviderScore(id: string, metrics: any = {}) {
    ...
    if (metrics.verified) trustScore += 10;
    ...
    if (metrics.onTimeRate) performanceScore += Math.min(15, Number(metrics.onTimeRate) / 10);
    if (metrics.disputes) {
    ...
    return (this.prisma as any).businessProviderProfile.update({
      where: { id },
      data: {
        trustScore,
        riskScore,
  ...
  async createOffer(data: any) {
    ...
    let aiScore = 50;
    if (data.price && Number(data.price) > 0) aiScore += 10;
    if (data.durationDays && Number(data.durationDays) <= 14) aiScore += 10;
    if (data.serviceType === "shipping") aiScore += 5;
    if (data.serviceType === "insurance") aiScore += 5;
    if (data.serviceType === "finance") aiScore += 5;

    aiScore = this.clamp(aiScore);

    return (this.prisma as any).providerOffer.create({
      data: {
  ...
  listOffers(serviceType?: string) {
    return (this.prisma as any).providerOffer.findMany({
      where: serviceType ? { serviceType } : {},
      orderBy: [{ aiScore: "desc" }, { createdAt: "desc" }],
    });
  }

  async matchProvider(data: any) {
    const provider = await this.findProvider(data.providerId);

    let score = 50;

    if (provider.type === data.serviceType) score += 20;
    if (provider.verified) score += 10;
    if (provider.country && provider.country === data.country) score += 10;
    if (provider.trustScore) score += Math.min(20, Number(provider.trustScore) / 5);
    if (provider.performanceScore) score += Math.min(15, Number(provider.performanceScore) / 7);
    if (provider.riskScore && Number(provider.riskScore) > 70) score -= 20;

    score = this.clamp(score);

    return (this.prisma as any).providerMatch.create({
      data: {
        dealId: data.dealId,
        providerId: data.providerId,
        serviceType: data.serviceType,
  ...
  listMatches() {
    return (this.prisma as any).providerMatch.findMany({
