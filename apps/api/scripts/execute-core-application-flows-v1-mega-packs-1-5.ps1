$ErrorActionPreference = "Stop"
Set-StrictMode -Version Latest

$ExpectedBranch = "feature/services-platform-v2"
$CommitMessage = "feat(api): core application flows v1 mega packs 1-5"
$ScriptRoot = $PSScriptRoot
$ApiRoot = Split-Path -Parent $ScriptRoot
$RepoRoot = (Resolve-Path (Join-Path $ApiRoot "..\..")).Path

Set-Location $RepoRoot

$currentBranch = (git branch --show-current).Trim()
if ($currentBranch -ne $ExpectedBranch) {
    throw "Expected branch '$ExpectedBranch' but found '$currentBranch'."
}

$requiredFiles = @(
    "apps/api/src/core-application-flows/core-flow.types.ts",
    "apps/api/src/core-application-flows/core-flow.utils.ts",
    "apps/api/src/quotes/quotes.module.ts",
    "apps/api/src/quotes/quotes.controller.ts",
    "apps/api/src/quotes/quotes.service.ts",
    "apps/api/src/orders/orders.module.ts",
    "apps/api/src/orders/orders.controller.ts",
    "apps/api/src/orders/orders.service.ts",
    "apps/api/src/invoices/invoices.module.ts",
    "apps/api/src/invoices/invoices.controller.ts",
    "apps/api/src/invoices/invoices.service.ts",
    "apps/api/src/payments/payments.module.ts",
    "apps/api/src/payments/payments.controller.ts",
    "apps/api/src/payments/payments.service.ts",
    "apps/api/src/reservations/reservations.module.ts",
    "apps/api/src/reservations/reservations.controller.ts",
    "apps/api/src/reservations/reservations.service.ts",
    "apps/api/src/workflows/workflows.module.ts",
    "apps/api/src/workflows/workflows.controller.ts",
    "apps/api/src/workflows/workflows.service.ts",
    "apps/api/src/events/events.module.ts",
    "apps/api/src/events/events.controller.ts",
    "apps/api/src/events/events.service.ts",
    "apps/api/scripts/verify-core-application-flows-v1-mega-packs-1-5.mjs",
    "apps/api/scripts/smoke-core-application-flows-v1-mega-packs-1-5.mjs",
    "apps/api/scripts/execute-core-application-flows-v1-mega-packs-1-5.ps1"
)

foreach ($file in $requiredFiles) {
    if (-not (Test-Path $file)) {
        throw "Missing required file: $file"
    }
}

Set-Location $ApiRoot

pnpm prisma generate
if ($LASTEXITCODE -ne 0) {
    throw "Prisma Client generation failed."
}

pnpm build
if ($LASTEXITCODE -ne 0) {
    throw "Core Application Flows V1 build failed."
}

$verificationJson = & node (Join-Path $ScriptRoot "verify-core-application-flows-v1-mega-packs-1-5.mjs")
if ($LASTEXITCODE -ne 0) {
    throw "Core Application Flows V1 verification failed."
}
$verificationJson | ConvertFrom-Json | Format-List

$smokeJson = & node (Join-Path $ScriptRoot "smoke-core-application-flows-v1-mega-packs-1-5.mjs")
if ($LASTEXITCODE -ne 0) {
    throw "Core Application Flows V1 smoke test failed."
}
$smokeJson | ConvertFrom-Json | Format-List

Set-Location $RepoRoot

git add -- $requiredFiles
if ($LASTEXITCODE -ne 0) {
    throw "git add failed."
}

git diff --cached --check
if ($LASTEXITCODE -ne 0) {
    throw "Git staged validation failed."
}

git commit -m $CommitMessage
if ($LASTEXITCODE -ne 0) {
    throw "git commit failed."
}

git push origin $ExpectedBranch
if ($LASTEXITCODE -ne 0) {
    throw "git push failed."
}

Write-Host "Core Application Flows V1 Mega Packs 1-5 completed successfully." -ForegroundColor Green
Write-Host "Restart API manually with: pnpm --filter api start:dev" -ForegroundColor Yellow
