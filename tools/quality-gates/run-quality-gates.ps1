...
Invoke-AvosTypeScript -RepoRoot $RepoRoot

Write-Host "`n[2/6] Workspace Build" -ForegroundColor Cyan
Invoke-AvosBuild -RepoRoot $RepoRoot
...
$smoke = if ($SmokeScript) {
    ...
    [pscustomobject]@{ status="NOT_APPLICABLE"; name="Smoke Tests" }
}
...
$integration = if ($IntegrationScript) {
    ...
    [pscustomobject]@{ status="NOT_APPLICABLE"; name="Integration Tests" }
}
...
$verification = if ($VerificationScript) {
    ...
    [pscustomobject]@{ status="NOT_APPLICABLE"; name="Verification" }
}
