export type CodeGenJsonPrimitive =
  | string
  | number
  | boolean
  | null;

export type CodeGenJsonValue =
  | CodeGenJsonPrimitive
  | CodeGenJsonValue[]
  | {
      [key: string]: CodeGenJsonValue;
    };

export type CodeGenMetadata = Record<
  string,
  CodeGenJsonValue
>;

export enum CodeGenKernelStatus {
  CREATED = "created",
...
export enum CodeGenEventSeverity {
  DEBUG = "debug",
...
export interface CodeGenEvent<TPayload extends
  CodeGenJsonValue = CodeGenJsonValue> {
  id: string;
  sequence: number;
...
export type CodeGenEventHandler<
  TPayload extends CodeGenJsonValue =
    CodeGenJsonValue,
> = (
  event: CodeGenEvent<TPayload>,
) => Promise<void> | void;
...
export enum CodeGenConfigurationSource {
  DEFAULT = "default",
  FILE = "file",
...
export enum CodeGenManifestType {
  SYSTEM = "system",
  ENGINE = "engine",
  PLUGIN = "plugin",
  BLUEPRINT = "blueprint",
...
export enum CodeGenPluginStatus {
...
export enum CodeGenDiagnosticLevel {
  DEBUG = "debug",
  INFORMATIONAL = "informational",
  WARNING = "warning",
  ERROR = "error",
