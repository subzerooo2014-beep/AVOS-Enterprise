...
foreach ($entity in @($bp.entities)) {
  ...
  $updateDtoFields = New-Object System.Collections.Generic.List[string]
  ...
  foreach ($field in @($entity.fields)) {
    ...
    if ($field.required -eq $false) {
      $dtoFields.Add("  $($field.name)?`: $tsType;")
    }
    else {
      $dtoFields.Add("  $($field.name)!`: $tsType;")
    }

    $updateDtoFields.Add("  $($field.name)?`: $tsType;")
  }
  ...
  $entityTokens.UPDATE_DTO_FIELDS = $updateDtoFields -join "`r`n"
...
$legacySpec = Join-Path $apiRoot "$moduleSlug.service.spec.ts"
