...
$ApiFeature=Join-Path $ApiRoot "src\global-enterprise-core"
$MobileFeature=Join-Path $MobileRoot "lib\src\features\global_enterprise_core"
...
try{
  ...
  node .\scripts\smoke-global-enterprise-core.mjs
  if($LASTEXITCODE -ne 0){throw "Smoke test failed"}

  node .\scripts\integration-global-enterprise-core.mjs
...
[pscustomobject]@{
  success=$true
  system="AVOS Ultra Bundle Z Global Enterprise Core"
  apiFiles=$ApiFiles
  ...
  globalGovernanceFramework=$true
  enterpriseCertificationFramework=$true
  standardsComplianceCenter=$true
  globalStandardsObservatory=$true
  executiveCommandCenter=$true
  enterpriseConstitutionalEngine=$true
  enterprisePolicyFramework=$true
  enterpriseControlTower=$true
  globalEnterpriseRegistry=$true
  enterpriseTrustNetwork=$true
  globalMonitoringCenter=$true
  enterpriseResilienceCenter=$true
  executiveStrategicConsole=$true
